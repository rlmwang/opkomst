 ...
 * Chapter composables.
 *
 * The chapters list is small (≤ dozens) so we always fetch the
 * full set; ``includeArchived`` just toggles the query string.
 * Mutations invalidate the list so the next render reads fresh
 * data.
 */
...
/** Sorted list, [] when loading. The SQL list isn't sorted (the
 * router doesn't ORDER BY) and the dropdown wants alphabetical. */
export function chapterList(
  query: ReturnType<typeof useChapters>,
): ComputedRef<Chapter[]> {
  return computed(() => (query.data.value ? sortByName(query.data.value) : []));
}
