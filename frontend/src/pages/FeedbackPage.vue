...
watch(
  ...
  (f) => {
    ...
    for (const q of f.questions) {
      if (q.kind === "rating") ratings.value[q.key] = null;
      if (q.kind === "text") texts.value[q.key] = "";
    }
  },
  { immediate: true },
);
...
async function submit() {
  ...
  for (const q of form.value.questions) {
    ...
    if (q.kind === "text" && !texts.value[q.key].trim()) {
      toasts.warn(questionPrompt(q));
      return;
    }
  }
  ...
  const answers = form.value.questions.map((q) =>
    q.kind === "rating"
      ? { question_key: q.key, answer_int: ratings.value[q.key] }
      : { question_key: q.key, answer_text: texts.value[q.key] || null },
  );
...
<template>
  <div class="container stack">
    ...
    <!-- ``submitted`` beats ``error``: once a submit lands, no
         downstream refetch should be able to flip the visitor
         back to a "link no longer valid" message. -->
    ...
    <template v-else>
      ...
      <form class="stack" novalidate @submit.prevent="submit">
        <AppCard v-for="q in form.questions" :key="q.key">
          <label class="prompt">
            {{ questionPrompt(q) }}
          </label>
          <RatingScale
            v-if="q.kind === 'rating'"
            :model-value="ratings[q.key]"
