...
<template>
  ...
  <div class="container">
    ...
    <AppCard v-else>
      ...
      <!-- Magic-link forms don't get a "save login" doorhanger
           from any browser: there's no password to store and no
           webauthn ceremony to bind to. The only autofill path
           that actually engages is Firefox's form-history (the
           double-click dropdown of previously-typed values),
           which keys on ``name`` + a recognised ``autocomplete``
           field-name. ``"email"`` is the right token for that;
           we deliberately don't claim ``"username webauthn"``,
           since neither side of that pair is doing any work. -->
