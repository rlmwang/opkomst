...
const live = $derived(Boolean(ads.client_id && slotId));
const support = supportLinks();

/** The ``<ins>`` itself, so the observer has something to watch. */
let unit = $state<HTMLElement | null>(null);
...
onMount(() => {
  ...
  if (!("IntersectionObserver" in window) || !unit) {
    request();
    return;
  }
...
<!-- The unit is never scaled or clipped: its size comes from requesting
     that ad format, which AdSense allows, rather than from transforming
     what it delivers, which it forbids. -->
{#if live}
  <ins
    bind:this={unit}
    class="adsbygoogle"
  ...
  ></ins>
{:else}
  <div class="filler {variant}">
    <span class="filler-text">{support.length > 0 ? c.supportHeading : c.adNone}</span>
    {#each support as s (s.url)}
      <a href={s.url} target="_blank" rel="noopener" class="support-link">
