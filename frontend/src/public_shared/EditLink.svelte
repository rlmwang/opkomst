 ...
 *  screen. The token lives only in this URL: there is no server-side way
 *  to recover it, so the copy is explicit that it cannot be re-sent. One
 *  component for all mini-apps.
 *
 *  It renders with no wrapper element, not as a card: its three lines
 *  drop straight into the single ``PublicConfirmation`` card as direct
 *  children, so they share that card's one column ``gap`` and the
 *  vertical rhythm stays even. The copy glyphs are inlined for the same
 *  reason the rest of the app's are: no icon font ships. */
...
async function copy(): Promise<void> {
  try {
    ...
    // link is still selectable on screen, so this is non-fatal.
    return;
  }
  copied = true;
...
<div class="link-row">
  ...
  >
      {#if copied}
        <svg viewBox="0 0 24 24" width="17" height="17" fill="currentColor" aria-hidden="true"><path d="M9,18.25A.74.74,0,0,1,8.47,18l-5-5A.75.75,0,1,1,4.53,12L9,16.44,19.47,6A.75.75,0,0,1,20.53,7l-11,11A.74.74,0,0,1,9,18.25Z" /></svg>
      {:else}
