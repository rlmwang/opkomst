...
const show = $derived(Boolean(ads) && !hide);
...
let wide = $state(window.matchMedia(RAILS_FROM).matches);

onMount(() => {
  if (!show) return;
  const media = window.matchMedia(RAILS_FROM);
...
{#if show && ads}
  {#if wide}
    ...
    <aside class="ad-rail ad-rail-left">
      {#if railLive}<span class="ad-label">{c.adLabel}</span>{/if}
      <div class="ad-box rail-box">
        <AdUnit {ads} slotId={ads.rail_slot} variant="rail" {locale} />
      </div>
    </aside>
  ...
  {:else}
    ...
    <aside class="ad-banner">
      {#if bannerLive}<span class="ad-label">{c.adLabel}</span>{/if}
      <div class="ad-box banner-box">
        <AdUnit {ads} slotId={ads.banner_slot} variant="banner" {locale} />
      </div>
    </aside>
  {/if}
{/if}
