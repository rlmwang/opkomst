 ...
 */
const { started, email }: { started: Started; email: string } = $props();

const toasts = useToasts();

function open() {
  window.open(started.public_url, "_blank", "noopener");
}

async function copy() {
  try {
    await navigator.clipboard.writeText(started.public_url);
...
<div class="container-wide">
  <AppCard>
    ...
    <div class="actions">
      <AppButton label={t("start.openIt")} severity="secondary" text onclick={open} />
      <AppButton label={t("start.copyLink")} icon="link" onclick={copy} />
    </div>
  </AppCard>
</div>
