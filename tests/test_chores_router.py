...
def _create(client: Any, headers: Any, **overrides: Any) -> Any:
    ...
    return client.post("/api/v1/chore", headers=headers, json=body)
...
def test_update_reconciles_chores(client, organiser_headers):
    ...
    r = client.put(f"/api/v1/chore/{created['id']}", headers=organiser_headers, json=body)
...
def test_archive_restore_delete_flow(client, organiser_headers):
    ...
    assert client.delete(f"/api/v1/chore/{rid}", headers=organiser_headers).status_code == 404
    ...
    assert client.post(f"/api/v1/chore/{rid}/archive", headers=organiser_headers).status_code == 404

    assert client.post(f"/api/v1/chore/{rid}/restore", headers=organiser_headers).status_code == 200
    # Archive then hard-delete.
    client.post(f"/api/v1/chore/{rid}/archive", headers=organiser_headers)
    assert client.delete(f"/api/v1/chore/{rid}", headers=organiser_headers).status_code == 204
    assert client.get(f"/api/v1/chore/{rid}", headers=organiser_headers).status_code == 404
...
def test_unknown_roster_is_404(client, organiser_headers):
    assert client.get("/api/v1/chore/nonexistent", headers=organiser_headers).status_code == 404
...
def test_roster_in_other_chapter_is_404_for_organiser(client, admin_headers, organiser_headers):
    ...
    assert client.get(f"/api/v1/chore/{roster['id']}", headers=organiser_headers).status_code == 404
...
def test_accountability_breaks_down_per_chore(client, organiser_headers):
    ...
    sections = client.get(f"/api/v1/chore/{roster['id']}/accountability", headers=organiser_headers).json()
...
def test_calendar_endpoint_returns_per_chore_days(client, organiser_headers):
    ...
    cal = client.get(f"/api/v1/chore/{roster['id']}/calendar", headers=organiser_headers).json()
...
def test_rebalance_preview_returns_calendar_with_changes(client, organiser_headers):
    ...
    roster = _create(client, organiser_headers, chores=[{"name": "Bar", "cycle_slots": [0, 1, 2, 3, 4, 5, 6]}]).json()
    client.post(f"/api/v1/chore/{roster['id']}/activate", headers=organiser_headers)  # window pins open
    client.post(
        f"/api/v1/chore/by-slug/{roster['slug']}/enroll",
        json={"display_name": "Ada", "chore_ids": [roster["chores"][0]["id"]]},
    )
    preview = client.get(f"/api/v1/chore/{roster['id']}/rebalance/preview", headers=organiser_headers).json()
