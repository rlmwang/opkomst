...
def test_feedback_does_not_fire_for_future_event(db: Any, fake_email: Any) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (EmailChannel.FEEDBACK, "feedback_enabled"),
    ],
)
def test_does_not_fire_when_event_toggle_off(
    ...
    assert mail_lifecycle.run_once(channel) == 0
...
@pytest.mark.parametrize("channel", _BOTH_CHANNELS)
def test_marks_sent_and_mints_message_id(channel: EmailChannel, db: Any, fake_email: Any) -> None:
    e = _make_event_in_window(db, channel)
    s = make_signup(db, e, email="alice@example.test")
    commit(db)

    mail_lifecycle.run_once(channel)
...
@pytest.mark.parametrize("channel", _BOTH_CHANNELS)
def test_failed_send_marks_failed_and_clears_message_id(channel: EmailChannel, db: Any, fake_email: Any) -> None:
    e = _make_event_in_window(db, channel)
    s = make_signup(db, e, email="alice@example.test")
    commit(db)
...
def test_reminder_retry_succeeds_on_second_attempt(db: Any, fake_email: Any) -> None:
...
@pytest.mark.parametrize("channel", _BOTH_CHANNELS)
def test_done_wipes_when_no_other_pending(channel: EmailChannel, db: Any, fake_email: Any) -> None:
    """Single-channel event: ciphertext wipes after the send."""
    other_off = {
        EmailChannel.REMINDER: {"feedback_enabled": False},
...
def test_decrypt_failure_flips_to_failed_once(channel: EmailChannel, db: Any, fake_email: Any) -> None:
    ...
    d = get_dispatch(db, s, channel)
    assert d is not None
    d.encrypted_email = b"not-real-ciphertext"
    db.add(d)
    commit(db)
...
@pytest.mark.parametrize("channel", _BOTH_CHANNELS)
def test_conditional_update_does_not_stomp_existing_status(channel: EmailChannel, db: Any, fake_email: Any) -> None:
    ...
    try:
        ...
        # against, but we still need locals to call into it.
        dispatch_id = d_a.id
        ...
        # ``_process_one`` takes primitives; the conditional
        # UPDATE in step 1 runs against DB truth and matches zero
        # rows because session B already deleted the row. The
        # function logs and returns without ever touching the ORM
        # object that's now stale.
