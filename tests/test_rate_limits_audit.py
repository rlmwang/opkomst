...
#
# Empty by default. ``backend/routers/dev.py`` lives outside this
# audit's scope: it's mounted only when ``settings.local_mode`` is
# True (which the test conftest forces False), so the router's
# routes never appear in the FastAPI app under test. No exemption
# entry needed.
...
def test_every_mutating_endpoint_is_rate_limited() -> None:
    ...
    assert not missing, "rate-limit gap detected — add @limiter.limit(...) to:\n" + "\n".join(
