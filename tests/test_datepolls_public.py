...
def test_public_get_returns_slots(client, organiser_headers):
    poll = _create(client, organiser_headers, ["2026-08-01", "2026-08-02"])
    r = client.get(f"/api/v1/datepoll/by-slug/{poll['slug']}")
...
def test_public_get_unknown_slug_410s(client):
    assert client.get("/api/v1/datepoll/by-slug/nope").status_code == 410


def test_public_get_archived_410s(client, organiser_headers):
    poll = _create(client, organiser_headers, ["2026-08-01"])
    client.post(f"/api/v1/datepoll/{poll['id']}/archive", headers=organiser_headers)
    assert client.get(f"/api/v1/datepoll/by-slug/{poll['slug']}").status_code == 410
...
def test_submit_happy_path_returns_edit_token(client, organiser_headers):
    ...
    r = client.post(
        ...
        json={
            "display_name": "Alex",
            "note": "works for me",
            "answers": [{"datepoll_slot_id": d0, "availability": "yes"}],
        },
    )
    ...
    assert subs[0]["note"] == "works for me"  # one note per submission, not per slot


def test_submit_unknown_slot_400s(client, organiser_headers):
    poll = _create(client, organiser_headers, ["2026-08-01"])
    r = client.post(
        f"/api/v1/datepoll/by-slug/{poll['slug']}/submit",
        json={
            "answers": [{"datepoll_slot_id": "not-a-real-id", "availability": "yes"}],
        },
    )
...
def test_submit_rate_limit_fires(client, organiser_headers):
    ...
    poll = _create(client, organiser_headers, ["2026-08-01"])
    d0 = poll["slots"][0]["id"]
    body = {"answers": [{"datepoll_slot_id": d0, "availability": "yes"}]}
    codes = [client.post(f"/api/v1/datepoll/by-slug/{poll['slug']}/submit", json=body).status_code for _ in range(21)]
    assert 429 in codes
