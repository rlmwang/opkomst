...
def _seed_minimal_event_and_signup() -> str:
    ...
    try:
        ...
        db.add(s)
        db.flush()
        # Dispatch row — the ``submit`` path expects one to exist
        # for the signup, even though these tests stop at form load.
        db.add(
            SignupEmailDispatch(
                signup_id=s.id,
...
def _mint_token(signup_id: str, *, expires_at: datetime) -> str:
    """Insert a feedback token row, return its raw value."""
    db = SessionLocal()
    try:
        raw = f"tok-{uuid7()}"
        db.add(
...
def _token_exists(raw: str) -> bool:
    ...
    try:
        return (
            db.query(FeedbackToken).filter(FeedbackToken.token == raw).first()
            is not None
        )
    finally:
        db.close()
